...
export function ToolLayout({ form, action, errors, result, className }: ToolLayoutProps) {
  return (
    <div className={cn("grid gap-6 lg:grid-cols-[minmax(0,20.5rem)_minmax(0,1fr)] lg:items-start lg:gap-10", className)}>
      <div className="space-y-3 lg:sticky lg:top-24 lg:self-start">
        <div className="divide-y divide-border/70 overflow-hidden rounded-xl border border-border/80 bg-surface/30">
          {form}
        </div>
        {action}
        {errors}
      ...
      <div className="min-w-0">
        {result ?? (
          <div className="flex min-h-[16rem] flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-border/70 p-8 text-center">
