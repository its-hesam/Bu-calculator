...
export interface TemplateCardsProps {
  ...
  bodies?: Partial<Record<"colleague" | "user", string>>
}
...
export function TemplateCards({ params, variant, bodies }: TemplateCardsProps) {
      ...
      <div className="space-y-4">
        {cards.map((card, i) => (
          <TemplateCard
            ...
            icon={cardIcons[card]}
            body={bodies?.[card] ?? templates[variant][card]}
            params={params}
...
function TemplateCard({ index, title, subtitle, icon, body, params, featured }: { index: number; title: string; subtitle: string; icon: ReactNode; body: string; params: Record<string, string>; featured: boolean }) {
  ...
  return (
    <div className={cn("card-glow overflow-hidden rounded-2xl border bg-card", featured ? "border-primary/30" : "border-border/80")}>
      <div className={cn("flex items-center gap-3 border-b px-6 py-4", featured ? "border-primary/15 bg-primary/[0.04]" : "border-border/60 bg-muted/20")}>
      ...
      <div className="flex flex-col gap-3.5 p-6">
        <textarea
          ...
          className="w-full flex-1 resize-y rounded-xl border border-input bg-input/40 px-4 py-3.5 text-sm leading-relaxed text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-ring/50"
        ...
        <div className="flex flex-wrap gap-1.5">
          {Object.entries(params).map(([k, v]) => (
            <span
              key={k}
              className="inline-flex items-center gap-1.5 rounded-lg border border-border/70 bg-surface/60 px-2 py-1 font-mono text-[11px] text-muted-foreground"
            >
